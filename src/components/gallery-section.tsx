          ...
          {dict.items.map((item, index) => (
            ...
            >
              ...
              <div className="absolute inset-x-0 bottom-0 flex items-end justify-between p-5">
                <div>
                  <span className="text-[10px] tracking-[.2em] text-violet-300">
                    0{index + 1}
                  </span>
                ...
                <span className="text-xs text-white/50">{dict.expand}</span>
              </div>
            </motion.button>
          ))}
        </div>
        <div className="mt-12 text-center">
          <ExternalLinkButton href={externalLinks.fantia} variant="ghost">
            {dict.viewMore}
          </ExternalLinkButton>
        </div>
      </div>
